...
let globalThemes = JSON.parse(getThemeData(REGISTERY_PATH)).map((reg) => reg.theme);
let result = [];

if (targetComponent !== '*') {
  result.push({[targetComponent]: getMissingThemes(targetComponent)});
} else {
  const componentMap = fs.readdirSync(CMP_PATH, {withFileTypes: true}).filter((file) => file.isDirectory()).map(file => file.name)
  
  for(const cmp of componentMap) {
    const missing = getMissingThemes(cmp);

    result.push({[cmp]: missing});
  }
}
...
function getMissingThemes(targetComponent) {
  let missingThemes = [];
  const targetPath = path.join(CMP_PATH, targetComponent, '/dependencies/style/themes.css');

  const rawCSS = getThemeData(targetPath);
  
  if(!rawCSS) {
    console.warn(`Skipping [${targetComponent.toUpperCase()}]: 'themes.css' not found.`);
